---
permalink: /
title: "About"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

> Welcome to my personal website. Use the links above to read about my talks, teaching activities and publications. See below for information about myself and some recent projects.
>
> <div style="text-align: right"> &mdash; Matthew </div>

<br/>

<div style="text-align: center;">
  <img src="../images/IMG_top.jpeg" style="width: 100%; height: 100px; object-fit: cover;">
</div>

<br/>

Projects
======

### Current
* Algorithms for highly-constrained university scheduling problems.

### Past
* Open-source hyper-heuristic framework for the [IHTC-2024](https://ihtc2024.github.io/)
  - [Pre-print](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5601273)
* Modelling and solving multi-objective university course timetabling problems
  - [Thesis](https://eprints.lancs.ac.uk/id/eprint/233619/)
  - [Modelling paper](https://link.springer.com/article/10.1007/s10951-024-00817-w)

<div style="text-align: center;">
  <img src="../images/IMG_bottom.jpeg" style="width: 100%; height: 100px; object-fit: cover;">
</div>

<br/>

Biography
======

Dr Matthew Davison is currently a postdoctoral fellow at Aarhus University in Denmark. He gained his MMath (First Class with Honours) from the University of Edinburgh and his PhD in Statistics and Operational Research from STOR-i Centre for Doctoral Training which is based at Lancaster University in the United Kingdom.

His research interests lie in the overlap of operational research, computer science and mathematics. In particular, he has been drawn to various NP-hard optimisation problems such as scheduling or timetabling problems. This has led to various positions within industry taking on consulting roles as well as data science roles.

In his spare time, Matthew enjoys cycling, bouldering, and football (both playing and watching).

<div style="text-align: center;">
  <a href="/mug/">
    <img src="../images/favicon-192x192.png" style="width: 100px; height: 100px;">
  </a>
</div>

